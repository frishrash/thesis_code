...
def exlasso(np.ndarray[floating, ndim=2, mode='c'] X_, int n_clusters,
                  float gamma=0.1,
   ...
   #initialize
    ...
    X_dup = np.tile(X_, n_clusters)  # X clone that will be used later

    conv = False
    
    H = X_.T.dot(np.linalg.pinv(F.T))
    cdef np.ndarray[floating, ndim=2, mode='c'] centers_ = np.ascontiguousarray(H.T) #init
    ...
    for iteration in range(max_iter):
        ...
        base_sum = ((X_.T - H.dot(F.T))**2).sum(axis=0)  # Sum per column
        ...
        for point_index in range(n_samples):
            ...
            curr_ind = indicators[point_index]
            ...
            res = X_dup[point_index].reshape(n_clusters, n_features)
            res = total_base_sum - base_sum[point_index] + ((res - H.T)**2).sum(axis=1)
            res2 = res + gamma * traces
            new_ind = res2.argmin()
            F_counts[new_ind] = F_counts[new_ind] + 1  # Update columns sums
            if (curr_ind != new_ind):
                conv = False
                F[point_index, curr_ind] = 0
                F[point_index, new_ind] = 1
        ...
        H = X_.T.dot(np.linalg.pinv(F.T))  # H = XF(F^TF)^-1

        # compute new centers
        new_centers = np.ascontiguousarray(H.T) # _centers_dense(X_, labels_, n_clusters, upper_bounds_)
        #print("new_centers:")
        #print(new_centers)
        #print("H.T:")
        #print(H.T)
        ...
        if verbose:
            print('Iteration %i, inertia %s, %f secs'
                  % (iteration, np.sum((X_ - centers_[labels]) ** 2), time.time() - start))
    ...
    if center_shift_total > 0:
        update_labels_distances_inplace(X_p, centers_p, center_half_distances,
                                        labels, lower_bounds, upper_bounds,
                                        n_samples, n_features, n_clusters)
    return centers_, F.nonzero()[1], iteration